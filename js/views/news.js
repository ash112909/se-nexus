...
const NEWS_TYPE_META = {
  ...
  warranty:  { label: 'Warranty',         color: '#1C3969', bg: '#E1F5EE', icon: 'ti-shield-check'   },
  ...
  Modal.show({
    ...
    body: `
      ...
      <div style="display:flex;gap:8px;padding-top:12px;border-top:0.5px solid #F0ECE8;">
        <button onclick="newsSave('${n.id}');Modal.close();" style="display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border-radius:8px;border:0.5px solid #E2DDD8;background:${saved?'#D6E4F7':'#FFFFFF'};color:${saved?'#1C3969':'#5A5F6E'};font-size:12px;font-weight:500;cursor:pointer;font-family:inherit;">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px;">
          ${['Inaccurate or incorrect information','Outdated — information no longer applies','Duplicate post','Inappropriate or irrelevant content','Missing critical information','Other'].map((reason, i) => `
            <label style="display:flex;align-items:center;gap:9px;padding:9px 12px;border:0.5px solid #E2DDD8;border-radius:8px;cursor:pointer;font-size:13px;color:#3A3D4A;">
              <input type="radio" name="report-reason" value="${reason}" ${i===0?'checked':''} style="accent-color:#1C3969;"/> ${reason}
            </label>`).join('')}
        </div>
    ...
    <div class="news-shell">
      <div class="news-filter-panel">
        ...
        <div class="nfp-section">
          ...
          <div class="nfp-item" id="nft-bulletin" onclick="newsFilter('type','bulletin')"><div class="nfp-item-dot" style="background:#1C3969;"></div>Service Bulletin</div>
